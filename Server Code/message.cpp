...
//		cmd = S_CMD_LIGHT_OFF;
		...
		//check if button is clicked that outputs "STOP" to the Socket
	...
	else if( strstr(m_tcp_buf->m_addr, "TURN") != 0)
	{
		int angle, speed;
		
		char temporary123[50];
		const char delimeter[2] = " ";
		
		strcpy(temporary123,m_tcp_buf->m_addr);

		char *token123;
		char *token1234;

		//sends String to turn: TURN ANGLE X SPEED Y
		token123 = strtok(temporary123,delimeter);	//"TURN"
		token123 = strtok(NULL,delimeter);	//"ANGLE"
		token123 = strtok(NULL,delimeter);	//angle "X"

		angle = atoi(token123);
		

		token1234 = strtok(NULL,delimeter);	//"SPEED"
		token1234 = strtok(NULL,delimeter);	//speed "Y"
		
		speed = atoi(token1234);


		printf("angle %d, speed %d",angle,speed);

		
		s_app->move_car_angle(speed, angle);	//move
	}
